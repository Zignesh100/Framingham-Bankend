...
export const MRupdate = async (req, res) => {
  try {
    const MrId = req.params.id;
    const updatedData = req.body;

    const updatedMr = await MRUserModel.findByIdAndUpdate(MrId, updatedData, {
      new: true,
    });

    if (!updatedMr) {
      return res.status(404).json({ error: "MR User not found" });
    }
